...
table(!rownames(seqtab) %in% rownames(samdf))
rownames(seqtab)[!rownames(seqtab) %in% rownames(samdf)]
...
sam <- speedyseq::psmelt(ps_bias) %>%
  ...
  dplyr::select(OTU, Sample_Name,Taxon, Abundance, target_subfragment) %>%
...
joint <- sam %>%
  ...
  dplyr::select(Sample_Name, Taxon, Abundance, Actual, Error, target_subfragment) %>%
  ungroup()
...
joint %>% 
  ...
  theme_bw()+
  labs(title = "Observed vs. Actual proportions (log-odds)") 

# Visualise the error in all pairwise ratios
joint %>%
  filter(Actual > 0) %>%
    compute_ratios(group_vars = c("Sample_Name", "target_subfragment")) %>%
    filter(!is.na(Actual)) %>%
...
lm_dat <- joint %>%
  dplyr::select(Sample_Name, target_subfragment, Taxon, Abundance, Actual) %>%
  ungroup() %>%
  filter(Actual > 0) %>%
  group_by(Taxon, target_subfragment)
...
boot_coefs <- boot_models %>% 
  ...
  bind_rows(.id="name") %>%
  tidyr::separate(name, into=c("Taxon", "target_subfragment"), sep=";")
...
fits <- joint %>%
  ...
  left_join(bias %>%
              dplyr::select(Taxon, target_subfragment, est_metacal=gm_mean, se_metacal=gm_se),
            by = c("Taxon", "target_subfragment")) %>%
  left_join(bias.lm %>% 
              dplyr::select(Taxon, target_subfragment, est_lm=estimate, se_lm=std.error), 
            by = c("Taxon", "target_subfragment")) %>%
  left_join(bias.lmboot %>% 
              dplyr::select(Taxon, target_subfragment, est_lmboot=.estimate, se_lmboot = se),
            by =c("Taxon", "target_subfragment")) %>%
  left_join(bias.clrboot %>% 
              dplyr::select(Taxon, target_subfragment, est_clrboot=.estimate, se_clrboot = se),
            by = c("Taxon", "target_subfragment")) %>%
...
# Comparison of primer replicates
