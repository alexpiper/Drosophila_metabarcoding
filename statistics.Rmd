...
# ANOVA testing influence of colelction method on replicate dissimilarity 
...
# See how well the model went on the training dataset
...
# The above step loses the sample metadata - Need to add it again
...
# Train model again on entire mock dataset then predict field samples
...
# Create ROC curve per target taxon
...
# Make a probability surface for the model decisions
...
# Check for any problematic detections
problem <- target_det %>% 
  filter(abundance  > 0) %>%
  filter(.pred == 1 & is.na(outcome))

# Check for disagreements with mock communities
diss <- final_predictions_glm %>% 
  mutate(diff = !outcome  == .pred) %>%
  filter(diff)

# COunt number of True positives and False negatives
target_det %>% 
  filter(abundance  >0) %>%
  mutate(success = case_when(
    outcome ==1 & .pred == 1 ~ "TP",
...
# Create plot of detection sensitivity
gg.sens <- target_det %>% 
  mutate(success = case_when(
    outcome ==1 & .pred == 1 ~ "TP",
    outcome == 1 & .pred == 0 ~ "FN",
...
# Filter out those observations that were predicted as false positives (index switching)
...
#Replace taxa predicted as False positives with zeroes
ps2 <- ps.merged
otu_table(ps2) <- otu_table(otutab * keeptab, taxa_are_rows = FALSE)

# Check taxa that were dropped
...
```{r Detection}
# Load in the pred_boots object that was created in the index_switching.RMD file
...
# Load expected data (from morphological counts)
exp <- read_csv("sample_data/expected_quant_merged.csv") %>%
  dplyr::rename(sample_name = Sample) %>%
  pivot_longer(-sample_name,
               names_to= "taxon",
...
#plot expected relative abundances
exp %>% 
...
#Get observed data (from sequencign reads)
...
# Add a pseudocount to avoid dividing by zero
...
# See how well bias estimates fit predict the observed data from the expected data
preds <- joint %>%
...
# NOTE: Bias estimates are missing for D. biarmipes in BF1-BR1 primers because it was under filtering threshold
```

## Multifig1

Comparison of primers for detection and bias
...
# Comparison of primer replicates for detection and bias

Miseq run 2 tested whether the twin tagging strategy works. Can use this data to see if there was difference in quantitative performance between primers.
...
## Insects trapped (morphological counts)
...
## Number of species (From sequencing)
...
sespd <- picante::ses.pd(as(phyloseq::otu_table(ps_run3), "matrix"),  phyloseq::phy_tree(ps_run3), null.model = "taxa.labels", include.root = F, runs = 999)
...
# Detected species match those predicted by breakaway - can go ahead with ANOVAs 
...
# Plot species richness boxplots
...
We will try a range of different distance metrics with different assumptions, and see what captures the data best
...
#Root phylogenetic tree for Philr distance
...
Test for differences between community types and trapping methods
```{r betatest}
# Adonis test
...
# Use model 4 as we see a significant interaction effectC
anova_res[[4]]$table

# Look at the significant species
univa_tests <- as_tibble(univa_res[[4]][["uni.test"]], rownames ="term" ) %>%
  pivot_longer(-term,
               names_to = "species",
               values_to="test") %>%
  left_join(
    as_tibble(univa_res[[4]][["uni.p"]], rownames ="term" ) %>%
      pivot_longer(-term,
                   names_to = "species",
                   values_to="pval") 
  ) %>%
