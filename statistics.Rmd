...
# Where is rep4-6 coming from?

## Just subset to the last run
seqtab.nochim <- readRDS("output/rds/seqtab_final.rds")
seqtab.nochim <- seqtab.nochim[str_detect(rownames(seqtab.nochim), "HLVKYDMXX"),]
seqtab.nochim <- seqtab.nochim[,colSums(seqtab.nochim) > 0]
...
ps <- phyloseq(tax_table(tax_plus), sample_data(samdf),
               otu_table(seqtab.nochim, taxa_are_rows = FALSE), phy_tree(phy))
...
sample_data(ps.merged) <- read.csv("sample_data/Sample_info.csv", header=TRUE) %>% 
  filter(FCID=="HLVKYDMXX") %>% 
  ...
  #mutate(Sample_Name = str_remove(Sample_Name, "^HLVKYDMXX_")) %>%
  mutate(type = case_when(
    str_detect(Sample_Name, "_D")  ~ "Mock",
    str_detect(Sample_Name, "SPD")  ~ "SPD",
    str_detect(Sample_Name, "ACV")  ~ "ACV",
    str_detect(Sample_Name, "DC")  ~ "DC",
    str_detect(Sample_Name, "FF")  ~ "FF",
    str_detect(Sample_Name, "NTC")  ~ "NTC",
    str_detect(Sample_Name, "POS")  ~ "POS",
    str_detect(Sample_Name, "CT")  ~ "CarpTrap",
    str_detect(Sample_Name, "CM")  ~ "CarpMock"
  )) %>%
...
# Phylogeny Other taxa

Comparison of attractants

```{r}
#ps2 <- speedyseq::tax_glom(ps.merged, taxrank="Species")
ps2 <- subset_samples(ps.merged, !str_detect(sample_names(ps.merged), "POS"))#Remove mocks
ps2 <- subset_samples(ps2, !str_detect(sample_names(ps2), "_D"))#Remove mocks
ps2 <- subset_samples(ps2, !str_detect(sample_names(ps2), "NTC"))#Remove mocks

# Filter
ps3 <- ps2 %>%
    transform_sample_counts(function (x) x/sum(x)) %>%  # Convert to proportions
    transform_sample_counts(function (x) (x > 0.0001) * 1) %>%
    filter_taxa( function(x) mean(x) > 0, TRUE) #Drop missing taxa from table
#ps2.carson <- prune_samples(sample_sums(ps2.carson)>0, ps2.carson) # Drop empty samples


taxa_names(ps3) <- paste0("SV", seq(ntaxa(ps3)),"-",tax_table(ps3)[,4])
plot_tree(ps3, label.tips = "taxa_names", ladderize="left", color="type") +coord_polar(theta="y") + scale_colour_brewer(palette="RdBu")
