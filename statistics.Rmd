...
gg.bias_fits <- preds %>%
  ...
  geom_text(aes(x=-7, y=-1, label=rmse), check_overlap = TRUE, inherit.aes = FALSE)+
...
gg.bias <- preds %>%
  ...
  ggplot(aes(x=target_subfragment, y=estimate-1, fill=target_subfragment)) +
  geom_col()+
  coord_flip() +
  facet_grid(taxon~.) +
  base_theme+
...
# Bias is missing some taxa for some primers? check
```
...
gg.reads_classified <- tax_plot_data  %>%
  ...
  geom_text_repel(data= tax_plot_data %>%
            ...
            aes(label = target_subfragment),
            direction = "y", hjust=-1, segment.alpha = 0)+
  base_theme +
...
fig1a <- gg.reads_classified / gg.run1pca + plot_layout(heights = c(1,2))
 
fig1 <- fig1a - gg.bias + plot_layout(widths = c(2,1)) + plot_annotation(tag_levels = 'A')
...
# get independant variables - for me this will be abundance and presence absence
body.site <- FTmicrobiome$X       

ncol(OTUTable) #same length - refers to column
length(body.site)
