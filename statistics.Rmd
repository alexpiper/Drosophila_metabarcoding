...
gg.bias <- preds %>%
  ...
  stat_interval(aes(color_ramp  = stat(level)), position=position_dodge(width=0.6))+
  stat_summary(fun = mean, size=0.2, position=position_dodge(width=0.6), colour="black")+
...
div_table_rare <- sespd_rare %>%
  ...
  left_join(sample_data(ps_rare) %>% 
              as("data.frame") %>%
              filter(!duplicated(sample_id)) %>%
              dplyr::select(amp_rep, sample_id, type, orchard),
            by = "sample_id")  
...
# Difference in alpha diversity between community types
...
report::report(aov(alpha ~orchard, data=div_table_rare))
broom::tidy(TukeyHSD(aov(alpha ~orchard, data=div_table_rare)))


report::report(aov(Shannon ~orchard, data=div_table_rare))
broom::tidy(TukeyHSD(aov(Shannon ~orchard, data=div_table_rare)))

# plot differences in alpha diversity
alpha_plotdat <- div_table_rare %>% 
  pivot_longer(2:7,
               names_to="metric",
               values_to="value") %>%
...
gg.alpha_rare_rich <- alpha_plotdat %>%
...
gg.alpha_rare_shan <- alpha_plotdat %>%
...
gg.alpha_rare <- gg.alpha_rare_rich / gg.alpha_rare_shan
...
#Plot betadispersal
z <- as.dist(distlist$WUnifrac_rare[metadata$sample_id, metadata$sample_id])
...
```{r pca plots}
# Get distance
plot_dist <- as.dist(distlist$WUnifrac_rare)
