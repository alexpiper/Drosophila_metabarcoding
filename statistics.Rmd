...
gg.run1pca <- plot_ordination(ps_run1, philr.pcoa, color = "extract_id", shape = "pcr_primers") +
...
colours_p3 <- p3$data %>%
  left_join(as_data_frame(sample_data(ps_run1)) %>%
              dplyr::select(sample_id, sample_name,  pcr_primers) %>%
  dplyr::rename(label = sample_id)
    )

p3 <- p3 %<+% colours_p3  + 
  geom_tippoint(aes(colour=pcr_primers, shape = pcr_primers), size=2)  + 
  geom_tiplab(aes(colour=pcr_primers, label=sample_name %>% str_remove("^.*_")), angle=90, hjust=1)+
  scale_colour_brewer(palette="Paired") +
...
gg.run1_heatmap <- ps_run1 %>%
  speedyseq::psmelt() %>%
  dplyr::select(sample_id, Abundance, pcr_primers, Species) %>%
...
gg.run1_heatmap_sep <- heatmap_dat %>%
  left_join(p1$data %>% dplyr::rename(Species = label)) %>%
...
p1 + gg.run1_heatmap_sep + plot_layout(widths = c(1,4))
...
gg.bias <- preds %>%
  ...
  ggplot(aes(x = taxon, y = estimate, colour = pcr_primers, group = pcr_primers)) +
...
#
fig1b <- gg.reads_classified / (gg.bias + coord_flip()) + plot_layout(heights = c(1,2))

fig1 <- fig1a - fig1b + plot_layout(widths = c(3,1)) + plot_annotation(tag_levels = 'A') & theme(plot.margin = unit(c(1,1,1,1), "mm"))
  
fig1 

fig1_alternate <- gg.run1_heatmap_sep / gg.bias + plot_layout(heights = c(2,1))+ plot_annotation(tag_levels = 'A') & theme(plot.margin = unit(c(1,1,1,1), "mm"))
