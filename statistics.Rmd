...
#Need to append the FCIDs before merging and removing chimeras

if they have REP and no HLVKYDMXX - Its run 2
if rhey have no rep its run 1
...
rownames(seqtab)[str_detect(rownames(seqtab), "Rep") & !str_detect(rownames(seqtab), "HLVKYDMXX")]<- paste0("CJKFJ_", rownames(seqtab)[str_detect(rownames(seqtab), "Rep") & !str_detect(rownames(seqtab), "HLVKYDMXX")])
...
joint <- sam %>%
  ...
  dplyr::select(Sample, Taxon, Abundance, Actual, Error) %>%
  ungroup()
...
joint %>% 
  filter(Actual > 0) %>%
  mutate_at(vars(Actual, Abundance), logit) %>%
  ggplot(aes(Actual, Abundance, color = Taxon)) + 
  geom_jitter(alpha=0.7) +
  scale_color_brewer(palette="Spectral")+
  theme_bw()+
  labs(title = "Observed vs. Actual (log-odds)") 

# Visualise the error in ratios
joint %>%
  filter(Actual > 0) %>%
    compute_ratios(group_vars = c("Sample")) %>%
    filter(!is.na(Actual)) %>%
    mutate(Pair = paste(Taxon.x, Taxon.y, sep = ":")) %>% 
  ggplot(aes(Pair, Error, colour=Pair)) +
  geom_jitter(alpha=0.7) +
...
bias.lm <- joint %>%
  ...
  mutate(Taxon = term %>% str_remove("Taxon"))

bias.lm

## Bootstrapped seperate regression models on proportions
library(tidymodels)
...
set.seed(606)
boots <- purrr::map(data, bootstraps, times=1000, apparant=TRUE)
...
ggplot(boot_aug, aes(x=Actual, y=Abundance, colour=Taxon)) +
...
## Bootstrapped seperate regression models on CLR
library(tidymodels)

clr_dat <- sam %>%
  dplyr::filter(Taxon %in% exp$Taxon) %>%
  left_join(exp, by = c("Sample","Taxon")) %>%
  mutate(Actual = replace_na(Actual, 0)) %>%
  dplyr::filter(Actual > 0) %>%
  mutate(Observed0 = (Abundance + 0.5) * (Actual > 0)) %>%
  group_by(Sample) %>%
  mutate(Abundance = clr(Observed0),
         Actual=clr(Actual)) %>%
  distinct() %>%
  dplyr::select(Sample, Taxon, Abundance, Actual) %>%
  ungroup() %>%
...
set.seed(606)
boots <- purrr::map(clr_dat, bootstraps, times=1000, apparant=TRUE)
...
ggplot(boot_coefs, aes(estimate)) +
  geom_histogram(bins = 30) +
  facet_wrap( ~ Taxon, scales = "free") +
  geom_vline(aes(xintercept = .estimate), data = bias.clrboot, col = "red") +
  geom_vline(aes(xintercept = .lower), data = bias.clrboot, col = "blue") +
  geom_vline(aes(xintercept = .upper), data = bias.clrboot, col = "blue")
...
rsq(boot_models, solubility, prediction)
...
fits <- joint %>%
  ...
  left_join(bias0 %>% dplyr::select(Taxon, est_metacal=Gm_mean, se_metacal=Gm_se), by = c("Taxon")) %>%
  left_join(bias.lm %>% dplyr::select(Taxon, est_lm=estimate, se_lm=std.error), by = c("Taxon")) %>%
  left_join(bias.lmboot %>% dplyr::select(Taxon, est_lmboot=.estimate, se_lmboot = se), by = c("Taxon")) %>%
  left_join(bias.clrboot %>% dplyr::select(Taxon, est_clrboot=.estimate, se_clrboot = se), by = c("Taxon")) %>%
  dplyr::mutate(est_obs = Abundance) %>%
  pivot_longer(starts_with("est_"),
              names_to = "Type",
              values_to= "Estimate") %>%
  mutate(Type = Type %>% str_remove("est_")) %>%
  mutate(Abundance = case_when(
    !Type=="obs" ~ Abundance / Estimate,
    Type=="obs" ~ Abundance))

std_err <- fits %>%
              dplyr::select(Taxon, se_metacal, se_lm, se_lmboot, se_clrboot) %>%
            pivot_longer(starts_with("se_"),
              names_to = "Type",
              values_to= "SE")%>%
  mutate(Type = Type %>% str_remove("se_"))
...
# Plot correlations
ggplot(fits, aes(x=Actual, y=Abundance)) +
  geom_point(aes(colour=Type, shape=Type), size=3, alpha=0.8, stroke=1) + 
  geom_abline(slope=1, intercept = 0) +
  ggpubr::stat_cor(aes(color=Type,label = paste(..rr.label.., ..p.label.., sep = "~`,`~")), label.x = 0.1)  + 
  xlim(0,1) + 
  ylim(0,1) + 
  ylab("Observed")
...
fits %>%
  ...
  filter(!Type=="obs") %>%
  distinct() %>%
  ggplot(aes(Taxon, y=Estimate-1, fill=Type, group=Type)) +
    geom_col(position=position_dodge(width=0.9))+
    geom_errorbar(aes(ymin=(Estimate-1) - SE, ymax=(Estimate-1) + SE), 
...
## Fit the mdoels again without bootstraps and quantify RMSE

## Find a way of quantifying the RMSE

```
we compared slopes to the 1:1 line (representing ideal prediction of recovered reads from input DNA) by taking the difference between the absolute value of the observed slope and 1. A slope of 1, i.e. a perfect association of read count template amount, would translate into a fold change of 0, a slope of 2 into a fold change of +1 and a slope of 0.5 into a fold change of −1.

Plot all the different regressions
