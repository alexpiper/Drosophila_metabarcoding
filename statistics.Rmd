...
dend <- hclust(philr.dist, method="ward.D2")

p3 <- ggtree(as.phylo(dend) ) + 
  theme_void()

colours_p3 <- p3$data %>%
  left_join(as_data_frame(sample_data(ps_run1)) %>%
              dplyr::select(sample_id, sample_name,  target_subfragment) %>%
  dplyr::rename(label = sample_id)
    )

p3 <- p3 %<+% colours_p3  + 
  geom_tippoint(aes(colour=target_subfragment, shape = target_subfragment), size=2)  + 
  geom_tiplab(aes(colour=target_subfragment, label=sample_name %>% str_remove("^.*_")), angle=90, hjust=1)+
  scale_colour_brewer(palette="Paired") +
    scale_x_reverse(expand=c(0, 20)) +
  theme(legend.position = "none") + coord_flip()

p3
# Plot as heatmap - Arrange by 
gg.run1_heatmap <- ps_run1 %>%
  speedyseq::psmelt() %>%
  dplyr::select(sample_id, Abundance, target_subfragment, Species) %>%
 ...
 # mutate(Abundance = metacal::clr(Abundance, na.rm = TRUE)) %>%
  left_join(p3$data %>% dplyr::select(sample_id = label, y)) %>%
    ggplot(aes(x=fct_reorder(sample_id,y), y=Species, fill=Abundance)) +
    geom_tile() +
  base_theme+
...
bias <- joint %>%
  ...
  group_by(target_subfragment, material_type)  %>%
    nest() %>%
    mutate(fit = purrr::map(data, ~lm(metacal::center_elts(observed0 /expected) ~ 0 + taxon , data = .)), 
...
boot_models <- joint %>%
  ...
  group_modify(~{
    ...
    mutate(fit =  purrr::map(splits, ~lm(metacal::center_elts(observed0 /expected) ~ 0 + taxon , data = .)), 
           coef =  purrr::map(fit, broom::tidy)) %>%
    int_pctl(coef) 
    }) %>%
  mutate(taxon = str_remove(term, "taxon")) %>%
...
gg.bias <- preds %>%
  ...
  geom_pointrange(aes(ymin = lower-1, ymax = upper-1), position=position_dodge(width=0.6))+
  coord_flip() +
  #facet_grid(target_subfragment~.) +
  base_theme+
  scale_colour_brewer(palette="Paired") +
...
gg.reads_classified <- tax_plot_data  %>%
  ggplot(aes(x=rank, y=value, fill = target_subfragment, group=target_subfragment)) +
...
fig1a <- p3 /gg.run1_heatmap + plot_layout(heights = c(1,2)) #/ (gg.run1pca+ theme(legend.position = "none"))
#
fig1b <- gg.reads_classified / gg.bias + plot_layout(heights = c(1,2))

fig1 <- fig1a - fig1b + plot_layout(widths = c(3,1)) + plot_annotation(tag_levels = 'A') & theme(plot.margin = unit(c(1,1,1,1), "mm"))
...
boot_models <- joint %>%
  ...
  group_modify(~{
    bootstraps(.x, times=1000, apparant=TRUE)  %>%
    mutate(fit =  purrr::map(splits, ~lm(metacal::center_elts(observed0 /expected, na.rm = TRUE) ~ 0 + taxon , data = .)), 
           coef =  purrr::map(fit, broom::tidy)) %>%
    int_pctl(coef) 
    }) %>%
  mutate(taxon = str_remove(term, "taxon")) %>%
...
ggplot(data=data)+
  geom_bar(mapping = aes(x=Week, y=as.numeric(Catch), fill=Attractant),stat="identity",position="dodge",colour="black") +
  facet_grid(Fruit~Orchard) + ylab("Total Catch") #

##Average catch per 2 weeks
ggplot(data=data)+
  geom_bar(mapping = aes(x=Week, y=Catch, fill=Attractant), stat="summary") +
  facet_grid(Attractant~Orchard) + ylab("Average catch for each 2 week period")
...
gg.alpha <- div_table %>% 
  ...
  geom_jitter(size=3, alpha=0.6, width=0.2, height=0)+
  geom_boxplot(fill=NA)+
  base_theme+
  scale_colour_brewer(palette="Paired") +
...
# Plot a heatmap of insect distances vs clustering of samp;les
...
p1dat <- speedyseq::psmelt(ps_run3) %>%
  dplyr::select(OTU, type, Abundance) %>%
...
gg.phylo <- gheatmap(p2, p2dat, offset=10, width=.2,
         colnames_angle=90, colnames_offset_y = .25, color="gray20") +
    scale_fill_viridis_c(option="A", name="Abundance", trans="log10")+ 
    geom_tiplab(offset=17, align=FALSE, size=3) 
