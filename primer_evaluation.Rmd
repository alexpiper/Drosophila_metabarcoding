...
file <- "primer_evaluation/amplicons/dgLCO1490_Uni-MinibarR1.fa.gz"
# Read in amplicon
...
#!/bin/bash
/usr/bin/ls -d $PWD/* | sed -e '1p' -e '/.fa.gz/!d' | sort -u > sequence_index.txt
```
...
# Run sumaclust for each ID level

#100%
/home/ap0y/sumaclust_v1.0.31/sumaclust -p 8 -t 1 $Sample.fa > $Sample.clustered_100.fa 
cat $Sample.clustered_100.fa  | grep '>' > $Sample.clustered_100.txt

#99%
/home/ap0y/sumaclust_v1.0.31/sumaclust -p 8 -t 0.99 $Sample.fa > $Sample.clustered_99.fa 
cat $Sample.clustered_99.fa  | grep '>' > $Sample.clustered_99.txt

#98%
/home/ap0y/sumaclust_v1.0.31/sumaclust -p 8 -t 1 $Sample.fa > $Sample.clustered_98.fa 
cat $Sample.clustered_98.fa  | grep '>' > $Sample.clustered_98.txt

#97%
/home/ap0y/sumaclust_v1.0.31/sumaclust -p 8 -t 1 $Sample.fa > $Sample.clustered_97.fa 
cat $Sample.clustered_97.fa  | grep '>' > $Sample.clustered_97.txt

#96%
/home/ap0y/sumaclust_v1.0.31/sumaclust -p 8 -t 1 $Sample.fa > $Sample.clustered_96.fa 
cat $Sample.clustered_96.fa  | grep '>' > $Sample.clustered_96.txt

#95%
/home/ap0y/sumaclust_v1.0.31/sumaclust -p 8 -t 1 $Sample.fa > $Sample.clustered_95.fa 
cat $Sample.clustered_95.fa  | grep '>' > $Sample.clustered_95.txt
...
sbatch --array=1-807 usearch_distmat.slurm
...
#USEARCH
usearch -calc_distmx $Sample.fa -tabbedout $Sample.txt -maxdist 0.05 -termdist 0.1 -threads 8

# Termdist and maxdist need to be changed as this produces massive files
...
# Check missing
complete <- list.files(path="primer_evaluation/amplicons/uclust_output/", pattern = ".txt.gz") %>%
  basename %>%
  str_remove(".txt.gz")
...
library(furrr)
plan(multiprocess)
mixed_clusters <- fs::dir_ls(path="primer_evaluation/amplicons/uclust_output/", glob = "*.txt.gz")%>%
...
# Plot out
lca_probs %>% 
  ggplot(aes(x=dist, y=prob, colour = rank, group = rank)) +
  geom_line() + 
  facet_wrap(~source)
...
mixed_clusters <- fs::dir_ls(path="primer_evaluation/amplicons/output/", glob = "*clustering.csv")%>%
  ...
  #purrr::map(function(x){
    ...
    all <- df %>%
      ...
      dplyr::summarise(mixed=n_distinct(tax_name)) %>%
      dplyr::mutate(type="all")
    pests <- df %>%
      dplyr::group_by(threshold) %>% 
...
#Test bad mixed
df <- vroom::vroom("primer_evaluation/amplicons/output/LCO1490_230-R_clustering.csv") 
...
joint <- vroom::vroom("primer_evaluation/amplicons/mixed_clusters_summary.csv", delim=",") %>%
  ...
  dplyr::select(Fname,Rname, threshold,mixed,type,seqs_amplified,spp_amplified,amplicon,combos,success)

# Summarise identification sucess by length on 2d scatter
gg.id <- joint %>% 
  dplyr::filter(threshold %in% c(0.97, 1)) %>%
  ggplot(aes(x=amplicon, y=success, colour=combos))+
