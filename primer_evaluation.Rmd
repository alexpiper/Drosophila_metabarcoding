...
dir.create("reference/pestorders/")
...
for (i in 1:length(pestlist)){
  ...
  if (length(subset) > 0){
    ...
    if (length(lengthfilt) > 0){insect::writeFASTA(lengthfilt,file=paste0("reference/pestorders/", pestlist[i],".fa.gz"),compress=TRUE)}
  }
}
...
dir.create("reference/pestfamilies/")
...
for (i in 1:length(pestlist)){
  ...
  if (length(subset) > 0){
    ...
    if (length(lengthfilt) > 0){insect::writeFASTA(lengthfilt,file=paste0("reference/pestfamilies/", pestlist[i],".fa.gz"),compress=TRUE)}
  }
}
...
Alignments <- sort(list.files("reference/pestfamilies/", pattern = ".fa.gz", full.names = TRUE)) # Read fasta filenames
