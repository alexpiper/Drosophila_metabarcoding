...
pdf(file="fig/Fig2.pdf", width = 11, height = 8 , paper="a4r")
  plot(Fig2)
try(dev.off(), silent=TRUE)
...
joint <- vroom::vroom("primer_evaluation/amplicons/mixed_clusters_summary.csv", delim=",") %>%
  ...
  mutate(success =  (spp_amplified - mixed)/spp_amplified,
         threshold = 1-dist) %>%
  dplyr::select(Fname,Rname, dist, threshold, mixed,type,seqs_amplified,spp_amplified,amplicon,combos,success) 
...
gg.id <- id_success %>%
  ggplot(aes(x=amplicon, y=success_alr, colour=combos))+
