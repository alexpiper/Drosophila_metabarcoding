...
dplyr::bind_rows(threshlist) %>%
  unnest_wider(taxa_in_cluster) %>%
  write_csv(paste0(name,"_clustering.csv"))
```
...
First generate artificial template sequence by concatenating both primer sequences with a 20 base spacer. This ensures that each primer will be treated separately in the BLAST search and thus achieves the equivalent effect of performing a separate BLAST search for each primer.
- Can you run a blastN with degenerate nucleotidess?
...
Could i instead calculate a kmerdistance to all the sequences in ncbi nt database, perhaps usign BBtools
