...
dir.create("primer_evaluation/pestorders/")
...
for (i in 1:length(pestlist)){
  ...
  if (length(subset) > 0){
    ...
    if (length(lengthfilt) > 0){insect::writeFASTA(lengthfilt,file=paste0("primer_evaluation/pestorders/", pestlist[i],".fa.gz"), compress=TRUE)}
  }
}
...
gg.primers <- ggplot(data=ent[ent$pos==1,], aes(x = as.numeric(pos))) +
  ...
  geom_density(data=ent %>% filter(rank %in% c(1:10)),
               aes(x=jitter(pos, factor=1, amount=1), fill=1), adjust = 1/2,  alpha=0.5, show.legend = FALSE)  + 
  geom_rug(data=ent %>% filter(rank %in% c(1:10)), aes(alpha=as.numeric(rank) / 10)) +
  geom_rug(data=primers %>% filter(Dir=="Forward"), aes(x= Stop, colour=Dir),size = 1) +
  geom_rug(data=primers %>% filter(Dir=="Reverse"), aes(x= Start, colour=Dir),size = 1) +
  scale_x_continuous(limits = c(min(primers$Start), max(primers$Stop)),breaks=seq(0,600,50),expand=c(0,0))  +
...
Fig1 <- gg.box / gg.density / gg.primers #+ plot_layout(heights=c(2,4, 1))
```
