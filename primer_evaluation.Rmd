...
for (i in 1:length(queries)){
 ...
 print(query)
  ...
  subset <- subset[lengths(subset)==712]
  ...
  if (nrow(subset) > 0){
    ...
    #%>% mutate(nd = nd / nseqs) # scale to relative
...
write_csv(ent_out, "primer_evaluation/ent_out.csv")
sw220_out <- bind_rows(sw220, .id="names")
write_csv(sw220_out, "primer_evaluation/sw220_out.csv")
sw420_out <- bind_rows(sw420, .id="names")
write_csv(sw420_out, "primer_evaluation/sw420_out.csv")
...
values <- read_csv("primer_evaluation/whole_alignment_entropy.csv")
...
# Test boxplot
library(ggthemes)
gg.box <- ggplot(ent %>% group_by(pos) %>% mutate(median = median(ma)), aes(x = pos, y=value, group=pos, colour=structure)) + 
...
### ID SUCCESS
  if (length(amplicon) > 3) {
    ...
    if (length(uniqx) > 1 ) {message(dat.passed$F.Name[p]," and ",  dat.passed$R.Name[p], " have mer slippage for ", name)}
