...
## Virtual PCR & Clustering ID sucess
...
#TESTING!
seqs <- seqs[1:1000]
combos <- combos %>% dplyr::slice(1:10)

#Get db
db <- taxreturn::get_ott_taxonomy(dir="ott3.2")
#make lists
ampedlist <- vector("list", length=nrow(combos))
lengthlist <- vector("list", length=nrow(combos)) 
clusterlist <- vector("list", length=nrow(combos)) 
...
for (p in 1:nrow(combos)) {
  ...
  clusterlist[[p]] <- dplyr::bind_rows(threshlist)
}

names(clusterlist) <- names(lengthlist) <- paste0(combos$Fname, "_",  combos$Rname)

bind_rows(clusterlist, .id="names") %>%
  unnest_wider(taxa_in_cluster) %>%
  write_csv("primer_evaluation/amplicons/clusterlist.csv")
