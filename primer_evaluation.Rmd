...
lengthfilt <- lengthfilt[lengths(lengthfilt)==712]
...
summaries <- dat %>%
  group_by(primer, Genus) %>% #Group by higher to minimize plotting
...
gg.mismatch <- ggplot(summaries, aes(x = primer, y = Genus , fill = sum)) +
  geom_tile() +
...
## Try ROTL get induced subtree to get tree of entire alignment. Query using tax_ids
...
problem - when subseqing, remove anythign with less than 100 non-gap characters
...
## TESTING
file <- "primer_evaluation/amplicons/jgLCO1490_BR3.fa.gz"
...
# Check missing
complete <- fs::dir_ls(path="primer_evaluation/amplicons/output/", glob = "*clustering.csv") %>%
  basename %>%
...
str_replace_all("Ill_C_R", "Ill-C-R") %>% #Can remove this with changed amplicon filenames
str_replace_all("Ill_B_F", "Ill-B-F")%>%
str_replace_all("C_LepFolR", "C-LepFolR")%>%
str_replace_all("ZBJ-ArtR2c_deg", "ZBJ-ArtR2c-deg")%>%
str_replace_all("ZBJ-ArtF1c_deg", "ZBJ-ArtF1c-deg")%>%
str_replace_all("230_R", "230-R") )

amplified <- fs::dir_ls(path="primer_evaluation/amplicons/", glob = "*.fa*") %>%
   purrr::map_dfr(function(x){
     ...
     all <- df %>%
      ...
      dplyr::summarise(seqs_amplified = n(), species_amplified = n_distinct(species)) %>%
      dplyr::mutate(type="all")
     pests <- df %>%
      dplyr::group_by(filepath) %>%
...
mutate(source = source %>%
str_replace_all("Ill_C_R", "Ill-C-R") %>%
str_replace_all("Ill_B_F", "Ill-B-F")%>%
str_replace_all("C_LepFolR", "C-LepFolR")%>%
str_replace_all("ZBJ-ArtR2c_deg", "ZBJ-ArtR2c-deg")%>%
str_replace_all("ZBJ-ArtF1c_deg", "ZBJ-ArtF1c-deg")%>%
...
joint <- mixed_clusters %>%
  ...
  left_join(combos) %>% 
  mutate(success =  seqs_amplified - mixed)
...
# Summarise identification sucess by length on 2d scatte


gg.id <- ggplot(joint, aes(x=amplicon, y=success, colour=combos))+
  geom_point()+
  #geom_text(aes(label=paste0(Fname, "_", Rname)))+
  facet_wrap(threshold~type, scales="free_y") +
...
sbatch --array=1-67 primerblast.slurm
