...
insect::writeFASTA(pruned, "reference/merged_final_1spponly.fa", compress = FALSE)

# Get shared tips between two trees
shared <- intersect(names(pruned), tree2$tip.label)
length(shared)
#38520 shared tips
...
constraints <- castor::extract_fasttree_constraints(tree2)

# Write out constraints as fasta file
Ntips <- length(tree2$tip.label)
cat(paste(sapply(1:Ntips, 
    FUN=function(tip) sprintf(">%s\n%s\n",tree$tip.label[tip],
    paste(as.character(constraints[tip,]),collapse=""))),collapse=""), "contstraints.fa")
```

# Align fasttree using constriants tree
```{bash fastree}
module load FastTree
FastTree -gtr -cat 20 -constraints constraints.fa -nt reference/merged_final_1spponly.fa > constraint_FastTree.nwk
```
