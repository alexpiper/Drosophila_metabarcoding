...
names(seqs) <- paste0(str_remove_all(str_extract(names(seqs), "^.*\\|" ), "[:punct:]"), str_extract(names(seqs), "\\|.*$" )) %>%
  str_replace("\\|\\|", "\\|")

# Then remove duplicate accession numbers (some with same accession have different taxonomy)
