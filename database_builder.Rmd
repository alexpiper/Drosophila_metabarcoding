...
#Filtered was then aligned in MAFFT - mafft folmer_insecta_full_withprimers.fa > folmer_insecta_fullength_withprimers_aligned.fa
...
# flag clusters with mixed taxonomy
#resolved <- insect::subset.DNAbin(resolved, subset = !duplicated(names(resolved)))
mixed_clusters <- taxreturn::get_mixed_clusters(x = resolved, rank = "species", threshold = 0.99, confidence=0.8, quiet = FALSE)

# Remove flagged sequences
mixed_clusters <- read_csv("reference/optimisation/resolved_mixedclusters.csv")

rem <- mixed_clusters %>% 
  filter(rank=="species", threshold ==0.99, confidence > 0.6) %>%
  pull(Acc)

purged  <- subset.DNAbin(resolved, subset = !str_replace(names(resolved), "(?:.(?!;))+$", "") %in% rem)
