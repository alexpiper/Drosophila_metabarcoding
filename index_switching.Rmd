...
```{r switch mocks}
ps <- readRDS("output/rds/ps1.rds")

# Subset to mocks
ps_model <- ps

tax_table(ps_model)[,8] <- tax_table(ps_model)[,8] %>% str_replace(" ", "_")
# Check Drosophila_suzukii in sample D500M1 - these are definitely misplaced and ruining the modelling
...
# Cross validations
mock_cv <- vfold_cv(mock_train, strata = fcid, v = 10)
...
switch_recipe_reps <- recipe(outcome ~ abundance_RA + fcid + n_extract + n_rep, data = mock_train) %>%
  ...
  step_novel(all_nominal(), -all_outcomes()) %>% 
  step_dummy(fcid, one_hot = TRUE)  %>% 
  #themis::step_tomek(outcome) %>%
  #step_mutate(abundance_RA = case_when(
  #  is.nan(abundance_RA) ~ 0,
  #  TRUE ~ abundance_RA
  #  )) %>%
  step_zv(all_predictors()) %>%
  step_log(abundance_RA, base=10, offset = 0.0001) %>%
...
# Check for disagreements within mock
test <- predictions_glm %>%
  dplyr::select(.pred_class, abundance_clr, abundance_RA, sample_id,  taxon, outcome)  %>%
  mutate(diff = !.pred_class == outcome)
...
set.seed(666)
boots <- bootstraps(mock_dat, times = 100, strata=outcome, apparent = FALSE)
