...
test <- ps %>%
  ...
  dplyr::select(Species, Abundance, switched, sample_id, type)
...
gg.bias_sep <- mock_dat %>%
  ggplot(aes(x = abundance_clr, fill = outcome)) +
...
glmnet_tune <- 
  tune_grid(glmnet_reps_wf, resamples = mock_cv, grid = glmnet_grid) 

autoplot(glmnet_tune)

glmnet_reps_wf <- glmnet_reps_wf %>%
  finalize_workflow(glmnet_tune %>% select_best("roc_auc" ))
...
boots <- bootstraps(mock_dat, times = 100, strata=fcid, apparent = TRUE)
  ...
  mutate(
    pred_logit = purrr::map(splits, function(x){
      ...
      mutate(filtered = case_when(
        ...
        dplyr::select(fcid, sample_id, sample_name, taxon, abundance_RA, outcome, filtered)
    }),
